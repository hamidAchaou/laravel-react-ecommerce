...
const RoleDetails = () => {
  ...
  const groupPermissions = (permissionNames) => {
    ...
    permissionNames.forEach(permissionName => {
      ...
      if (!groups[resource]) {
        const displayName = resource.split('_')
          .map(word => word.charAt(0).toUpperCase() + word.slice(1))
          .join(' ');
          ...
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
            <Box >
              ...
              <Typography variant="h6" color="text.secondary" component="p">
                Role Details & Permissions Management
              </Typography>
            </Box>
          </Box>
                ...
                <CardContent sx={{ p: 4 }}>
                  ...
                  <Grid container spacing={3}>
                    <Grid item xs={12} md={6}>
                      <Box sx={{ p: 3, border: 1, borderColor: 'divider', borderRadius: 2, bgcolor: 'background.default' }}>
                        <Typography variant="subtitle2" color="text.secondary" gutterBottom>
                          Role Name
                        </Typography>
                        <Typography variant="h6" fontWeight={600} sx={{ fontFamily: 'monospace', color: 'primary.main' }}>
                          {role.name}
                        </Typography>
                      </Box>
                    </Grid>
          ...
          >
            <CardContent sx={{ p: 4 }}>
              <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 4 }}>
                <Typography variant="h5" component="h2" fontWeight={600} sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
              ...
              {!role.permissions || role.permissions.length === 0 ? (
                <Alert severity="info" sx={{ borderRadius: 2 }}>
                  <Typography>
                    This role has no permissions assigned. Users with this role will have basic access only.
                  </Typography>
                </Alert>
              ) : (
                <Box>
                  <Alert severity="success" sx={{ mb: 4, borderRadius: 2 }}>
                    <Typography>
                      This role has <strong>{totalPermissions}</strong> permissions across{' '}
                    ...
                    {permissionGroups.map((group, groupIndex) => (
                      ...
                      >
                        ...
                        <Grid container spacing={2}>
                          {group.permissions.map((permissionName, permissionIndex) => {
                            const action = getActionFromPermission(permissionName);
                            const actionColor = getActionColor(action);
                            const displayName = formatPermissionName(permissionName);
                            
                            return (
                              <Grid item xs={12} sm={6} md={4} key={`${groupIndex}-${permissionIndex}`}>
                                <Box
                                  sx={{
                                    p: 2,
                                    borderRadius: 2,
          ...
          {role.permissions && role.permissions.length > 0 && (
            <Alert severity="warning" sx={{ borderRadius: 2 }}>
              <Typography variant="body2">
                <strong>Warning:</strong> This role has {role.permissions.length} permissions assigned. 
                Users with this role will lose access to these features.
              </Typography>
            </Alert>
          )}
        </DialogContent>
